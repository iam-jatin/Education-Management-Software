...
if(isset($_GET['civilyear'])){
	...
	$result = "SELECT * FROM all_students WHERE branch='Civil Engg.' && year='$civilyear'";
    $res = mysqli_query($conn, $result);
}

if(isset($_GET['cseyear'])){
	$cseyear = mysqli_real_escape_string($conn , $_GET['cseyear']);
	$result = "SELECT * FROM all_students WHERE branch='CSE' && year='$cseyear'";
    $res = mysqli_query($conn, $result);
}

if(isset($_GET['mechanicalyear'])){
	$mechanicalyear = mysqli_real_escape_string($conn , $_GET['mechanicalyear']);
	$result = "SELECT * FROM all_students WHERE branch='Mechanical Engg.' && year='$mechanicalyear'";
    $res = mysqli_query($conn, $result);
}

if(isset($_POST['submit'])){
    $roll_no = $_POST['roll_no'];
    for ($i=0;$i<sizeof ($roll_no);$i++) {  
$sql="INSERT INTO attendance (roll_no) VALUES ('$roll_no[$i]')";
...
<head>
    ...
    <title>Engineers Pro - Take Attendance</title>
                            ...
                            ?>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <input class="form-check-input" name="roll_no[]" type="checkbox" value="<?php echo $row["roll_no"]; ?>" id="flexCheckDefault">
                                            <label class="form-check-label" for="flexCheckDefault">Present</label></div></td>
                                        </tr>
										<?php
                                             $i++;
                                        } ?>
