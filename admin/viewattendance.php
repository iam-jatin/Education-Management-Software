...
if(isset($_GET['civilyear'])){
	$civilyear = mysqli_real_escape_string($conn , $_GET['civilyear']);
	$result = "SELECT all_students.*, attendance.* FROM all_students, attendance WHERE all_students.roll_no=attendance.roll_no AND DAY(attendance.time) = DAY(NOW()) AND branch='Civil Engg.' && year='$civilyear'";
    $res = mysqli_query($conn, $result);
}

if(isset($_GET['cseyear'])){
	$cseyear = mysqli_real_escape_string($conn , $_GET['cseyear']);
	$result = "SELECT all_students.*, attendance.* FROM all_students, attendance WHERE all_students.roll_no=attendance.roll_no AND DAY(attendance.time) = DAY(NOW()) AND branch='CSE' && year='$cseyear'";
    $res = mysqli_query($conn, $result);
}

if(isset($_GET['mechanicalyear'])){
	$mechanicalyear = mysqli_real_escape_string($conn , $_GET['mechanicalyear']);
	$result = "SELECT all_students.*, attendance.* FROM all_students, attendance WHERE all_students.roll_no=attendance.roll_no AND DAY(attendance.time) = DAY(NOW()) AND branch='Mechanical Engg.' && year='$mechanicalyear'";
    $res = mysqli_query($conn, $result);
}
...
<head>
    ...
    <title>Engineers Pro - View Attendance</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800">Today's Attendance</h1>
					</div>
					 <p class="mb-4">Students from selected branches.</p>
